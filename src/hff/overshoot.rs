...
pub struct Overshoot {
	pub scale: f64 ,
	pub direction:     i64,
	pub startExtremum: f64,
	pub peakExtremum:  f64,
	pub current:       f64,
...
impl Spectrum {
    pub fn new(scales: Vec<f64>) -> Self {
        Self { overshoots: scales.iter().map(|scale| (*scale, Overshoot::new(*scale)) ).collect() }
    }
...
impl Overshoot {
    ...
    pub fn maxOS(&self) -> f64 {
    	100.0 * (self.peakExtremum - self.startExtremum) / self.startExtremum / self.scale
    }
    pub fn reversal(&self) -> f64 {
        100.0 * (self.current - self.peakExtremum) / self.peakExtremum / self.scale
    }
    ...
    pub fn update(&mut self, x: f64) {
        //new := *ovs
        let cos = 100.0 * (x - self.startExtremum) / self.startExtremum / self.scale;
        let eDist = 100.0 * (x - self.peakExtremum) / self.peakExtremum / self.scale;
        let maxOS = self.maxOS();
        // if reversal...
        if cos*eDist < 0.0 && eDist.abs() > 1.0 {
            self.direction = -self.direction;
            self.startExtremum = self.peakExtremum;
            self.peakExtremum = x;
            self.current = x;
